...
Flags: 标志或选项
  ...
  -output string
		...
		输出文件名, 默认:srcdir/<type>_string.go
  -tags string
...
func main() {
	log.SetFlags(0)             //设置日志输出为: 标准输出?
	log.SetPrefix("stringer: ") //设置日志前缀
	log.Printf("开始执行")
	flag.Usage = Usage
	flag.Parse()
	if len(*typeNames) == 0 { //类型个数为0(没有提供类型)
		flag.Usage()
		log.Printf("请输入参数")
		os.Exit(2)
	}
	...
	// 接受目录或者文件列表, 默认使用当前目录, 参考:go run ../stringer.go -type=Pill .
	args := flag.Args()
	log.Printf("目录或文件列表参数args:%v", args)
	if len(args) == 0 {
...
// 发生器保存分析状态, 保存在内存buffer中的源代码格式化后的结果
